...
if __name__ == "__main__":
    ...
    top_oily = get_top_products(profile_oily, n=5)
    _print_top("oily only", profile_oily, top_oily)

    # Normal (default): all 0 -> normal=0.5
    profile_normal = user_input_to_profile()
    top_normal = get_top_products(profile_normal, n=5)
    _print_top("normal default", profile_normal, top_normal)

    profile_1 = {"dry": 0.9, "normal": 0, "oily": 0, "pigmentation": 0.5, "sensitive": 0.5, "wrinkle": 0.3}
    top_1 = get_top_products(profile_1, n=5)
    _print_top("profile 1", profile_1, top_1)
